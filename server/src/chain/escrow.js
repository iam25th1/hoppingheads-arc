 ...
 *
 * Config from env: ESCROW_ADDRESS, ARC_RPC_URL, CHAIN_ID (5042002 testnet).
 * Without ESCROW_ADDRESS the Arena is offline and says so; nothing here is
 * ever called for a sandbox lobby.
 */
...
export function chainConfig() {
  ...
  if (!escrow || !isAddress(escrow)) return null;
  return {
    chainId: Number(process.env.CHAIN_ID) || ARC_TESTNET_CHAIN_ID,
